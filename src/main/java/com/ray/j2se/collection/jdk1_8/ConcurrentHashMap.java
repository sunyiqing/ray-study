...
public class ConcurrentHashMap<K,V> extends AbstractMap<K,V>
    ...
    /**
     * The number of bits used for generation  stamp insizeCtl.
    ...
    static final int HASH_BITS = 0x7fffffff; // usable bits of normal node hash  //‭01111111111111111111111111111111‬
    ...
    //利用cas算法，设置i位置上的node节点，将c与table[i]比较，相同则插入v
     ...
     * -1代表正在初始化
     * -n表示用n-1线程正在进行扩容
     * 正数或者0代表hash 没有被初始化，这个数值表示初始化或下一次进行扩容大小
     * 类似于扩容阈值，它的初始值是中是 0.75倍这和loadfator对应的，实际容量>=sizeCtl
     */
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        ...
        for (Node<K,V>[] tab = table;;) {
            ...
            if (tab == null || (n = tab.length) == 0)
                tab = initTable();
            else if ((f = tabAt(tab, i = (n - 1) & hash)) == null) {//根据当前key定位出的node,如果为空表示当前位置可以写入数据利用CAS尝试写入，失败则自旋保证成功
                ...
                if (binCount != 0) {
                    if (binCount >= TREEIFY_THRESHOLD) //大于，转为红黑树
                        treeifyBin(tab, i);
                    if (oldVal != null)
                        return oldVal;
                    break;
    ...
    private final Node<K,V>[] initTable() {
        ...
        while ((tab = table) == null || tab.length == 0) {
            ...
            //如果sc==-1,也就是说正在初始化
                ...
                try {
                    //如果tab==null || table长度==0，则赋值初始化值16
                    if ((tab = table) == null || tab.length == 0) {
                        ...
                        table = tab = nt;
                        sc = n - (n >>> 2);//这里sc=12则表示，下一次扩容的大小
                    }
                } finally {
                    sizeCtl = sc;//将下一次库容数据赋值给sizeCtl
                }
                break;
            }
        }
     ...
     * 扩容相关的代码，类似于hashMap resize
     *
     * @param size number of elements (doesn't need to be perfectly accurate)
     */
    private final void tryPresize(int size) {
        //给定容量如果>=最大的一半，则库容到最大，否则调用函数扩容
        int c = (size >= (MAXIMUM_CAPACITY >>> 1)) ? MAXIMUM_CAPACITY :
            tableSizeFor(size + (size >>> 1) + 1);
        int sc;
        while ((sc = sizeCtl) >= 0) {//sc>=0说明还没有被库容，或者没有被初始化
            Node<K,V>[] tab = table; int n;
            //如果未被初始化
            if (tab == null || (n = tab.length) == 0) {
                n = (sc > c) ? sc : c;  //扩容阈值,取较大值
                //期间没有其他线程对表操作，则cas将sizectl状态设置为-1,表示正在初始化
                if (U.compareAndSwapInt(this, SIZECTL, sc, -1)) {
                    try {
                        if (table == tab) {
                            ...
                            Node<K,V>[] nt = (Node<K,V>[])new Node<?,?>[n];
                            table = nt;
                            sc = n - (n >>> 2);//无符号移动，相当于n*0.75,这里太牛逼了。哈哈
                        }
                    } finally {
                        sizeCtl = sc;//更新扩容阈值
                    }
